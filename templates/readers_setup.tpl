{config_load file='smarty.conf'}
	...
	<title>{#appnamefull#} | Setup {$reader.name}</title>
</head>
...
{include file='body_start.tpl'}
  ...
  <div class="content-wrapper">
    <section class="content-header">
      <h1>
        Setup reader
        <small>Configure the reader</small>
      </h1>
      <ol class="breadcrumb">
        <li><a href="/"><i class="fa fa-dashboard"></i> Dashboard</a></li>
        <li><a href="/readers"><i class="fa fa-tablet"></i> Readers</a></li>
        <li><a href="/readers/{$reader.id}"><i class="fa fa-server"></i> {$reader.name}</a></li>
        <li><a href="/readers/{$reader.id}/setup"><i class="fa fa-cog"></i> Setup</a></li>
      </ol>
    </section>
    ...
    <section class="content container-fluid">
      <div class="box">
        ...
        <div class="box-body">
		<div id="wizard_step1">
			<p>To setup your reader complete this wizard, download the configuration file to the reader's SD card and then power on the reader.</p>
			<p>OR power on the reader and follow the instructions shown.</p>
		...
		<div id="wizard_step3" hidden>
			...
			<span>Once downloaded, remove the SD card, insert it into the reader and power it up.</span><br />
			<span>After approximately 5 minutes the readers status should be showing as READY</span><br /><br />
			...
			<br /><a id="configurationdownloadlink" class="btn btn-primary" download="readerConfiguration.json">Configuration download</a>
		</div>
        </div>
        <!-- /.box-body -->
      </div>
	...
	function nextstep() {
		switch (wizard_step) {
			...
			case 2:
				...
				readerconfiguration={};
				readerconfiguration.readerid={$reader.id};
				readerconfiguration.refreshtoken="{$reader.refreshtoken}";
				readerconfiguration.connectiontype=$("input[type='radio'][name='connectiontype']:checked").val();

                                if($("input[type='radio'][name='connectiontype']:checked").val()=="wifi") {
					readerconfiguration.wifi={};
					readerconfiguration.wifi.ssid=$('#ssid').val();
					readerconfiguration.wifi.password=$('#password').val();
				};

				$('#configurationdownloadlink').attr('href', 'data:application/json;charset=utf-8,' + encodeURI(JSON.stringify(readerconfiguration)));
				$('#wizard_step2').hide();
				$('#wizard_step3').show();
				wizard_step=3;
				break;
